...
hsp = surf(linspace(-radius,radius,2*radius+1),linspace(-radius,radius,2*radius+1),zeros(round(2*radius+1)));
...
xd = round(subX); % Column coordinate
yd = round(subY); % Row coordinate
zd = round(subZ);%Slice coordinate
